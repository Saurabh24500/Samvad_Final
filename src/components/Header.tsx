...
export default function Header() {
  ...
  return <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border/50">
      <div className="container mx-auto px-4">
        ...
        {/* Mobile Navigation */}
        {mobileMenuOpen && <div className="md:hidden py-4 border-t border-border/50 animate-slide-up">
            <nav className="flex flex-col gap-3">
              {user ? <>
                  <Link to="/dashboard" className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-muted transition-colors" onClick={() => setMobileMenuOpen(false)}>
